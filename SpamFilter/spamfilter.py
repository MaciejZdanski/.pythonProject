...
Original file is located at
    https://colab.research.google.com/drive/1_xEt3gvY8Tzus2PjChK_jkQjl4_OVo9M

### Maciej Zdański

# Treść zadania:
Tematem projektu będzie napisanie modelu filtru spamowego wykorzystując do tego celu dostępne przykładowe dane, oraz używając narzędzi big data (apache spark). Do zbudowania modelu należu użyć zbioru danych znajdująceych się pod tym linkiem: https://www.kaggle.com/datasets/jerryanggara6/spam-filter-dataset.
Podstawowym założeniem jest korzystanie z modułów sparka do wczytywania, modyfikacji i manipulacji danymi. Filtr spamu trzeba zaimplementować za pomocą przetwarzania języka naturalnego zwanym analiza sentymentów. Dane należy na początek oczyścić i podzielić na dwie klasy (spam i nie spam), oraz poddać tokenizacji. Następnie stworzyć tzw. model worka słów czyli modyfikacja tekstu na numeryczny wektor cech. Kolejnym etapem będzie zmniejszenie wagi mniej istotnym słowom. Następnie należy przygotować kilka modeli (przykładowo regresja logistyczna, naiwny klasyfikator Bayesa). Stworzyć podział na zbiór
treningowy i testowy (najlepiej zrobić to za pomocą kroswalidacji. Ocenić jakość modelu za pomocą różnych metryk (accuracy, F1-score, …). Dobrze dodać jakąś wizualizację w postaci macierzy pomyłek i innych wykresów. Aby nie było to za trudne, można to zrobić na Możecie Państwo skorzystać z rozdziału 8 z książki “Python Machine Learning” Sebastiana Raschki, w której opisana jest analiza sentymentów i jak ją zrobić. Najważniejsze żeby tam gdzie to możliwe
były użyte moduły sparka (przykładowo nie korzystamy z Data Frame Pandas tylko Data Frame ze sparka). Projekt powinien zawierać możliwość przetestowania modelu (wpisanie własnego tekstu i sprawdzenie czy jest to spam czy nie). Ocena z projektu będzie podawana po indywidualnej rozmowie o projekcie na ostatnich zajęciach.
"""
